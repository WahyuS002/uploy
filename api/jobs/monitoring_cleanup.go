...
func cleanupMonitoringData(ctx context.Context) {
	...
	if err != nil {
		log.Printf("Monitoring cleanup: list servers: %v", err)
		return
	}
	for _, server := range servers {
		...
		if err != nil {
			log.Printf("Monitoring cleanup: connect server=%s: %v", server.ID, err)
			continue
		}
		err = monitoring.DeleteLocalData(ctx, client)
		client.Close()
		if err != nil {
			log.Printf("Monitoring cleanup: delete data server=%s: %v", server.ID, err)
			continue
		}
		if err := db.ClearServerMonitoringData(ctx, server.ID); err != nil {
			log.Printf("Monitoring cleanup: clear database server=%s: %v", server.ID, err)
		}
	}
}
