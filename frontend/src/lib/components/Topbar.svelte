...
>
	<a
		href="/projects"
		class="flex flex-none items-center gap-2 rounded-md px-2 py-1.5 select-none"
	>
		<span
	...
	<div class="flex flex-none items-center gap-1.5">
		...
		<DropdownMenu.Root>
			...
			<DropdownMenu.Portal>
				...
				>
					<div class="truncate px-2 py-1.5 text-xs text-muted-foreground select-none" title={userEmail}>
						{userEmail || 'Signed in'}
					</div>
					...
					<DropdownMenu.Item
						onSelect={onLogout}
						disabled={loggingOut}
						class="flex cursor-pointer items-center gap-2 rounded-md px-2 py-1.5 text-sm text-foreground outline-none data-highlighted:bg-surface-muted data-disabled:cursor-not-allowed data-disabled:opacity-60"
					>
