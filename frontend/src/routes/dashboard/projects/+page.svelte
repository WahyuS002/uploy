	...
	{#if showCreateForm}
		<div class="mb-6 rounded-lg border bg-gray-50 p-4">
			...
			>
				...
				<button
					type="button"
					onclick={() => { showCreateForm = false; createError = ''; }}
		...
		<div class="mb-4 flex items-center justify-between text-sm text-gray-500">
			<div class="flex items-center gap-2">
				...
				<label class="flex items-center gap-1">
					Sort By:
					<select bind:value={sortBy} class="cursor-pointer border-none bg-transparent p-0 text-sm text-gray-500 focus:ring-0">
			...
			<div class="flex items-center gap-1">
				<button
					...
					class="cursor-pointer rounded p-1 {viewMode === 'grid' ? 'bg-gray-200 text-black' : 'text-gray-400 hover:text-gray-600'}"
					title="Grid view"
				>
				...
				<button
					...
					class="cursor-pointer rounded p-1 {viewMode === 'list' ? 'bg-gray-200 text-black' : 'text-gray-400 hover:text-gray-600'}"
					title="List view"
				>
					<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
		...
		<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
			{#each sortedProjects() as project (project.id)}
				...
				>
					...
					<div class="relative mx-4 mb-3 flex h-28 items-center justify-center rounded-lg bg-gray-900"
					...
					<div class="flex items-center gap-2 border-t px-4 py-3 text-xs text-gray-500">
						{#if firstEnv}
							<span class="flex items-center gap-1">
								<span class="inline-block h-2 w-2 rounded-full {svcCount > 0 ? 'bg-green-500' : 'bg-gray-300'}"></span>
								{firstEnv.name}
							</span>
				...
				>
					<div class="flex items-center gap-3">
						...
						<div>
							...
							<p class="text-xs text-gray-400">Updated {new Date(project.updated_at).toLocaleDateString()}</p>
						</div>
					</div>
